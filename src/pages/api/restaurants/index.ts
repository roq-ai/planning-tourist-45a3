...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getRestaurantsPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'restaurants');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.restaurants.count(countOptions as unknown),
  ...
  async function getRestaurants() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.restaurants
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'restaurants'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
