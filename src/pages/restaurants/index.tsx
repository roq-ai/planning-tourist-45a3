...
import { getRestaurants, deleteRestaurantsById } from 'apiSdk/restaurants';
import { RestaurantsInterface } from 'interfaces/restaurants';

type ColumnType = ColumnDef<RestaurantsInterface, unknown>;
...
export function RestaurantsListPage(props: RestaurantsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RestaurantsInterface>>(
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteRestaurantsById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: RestaurantsInterface) => {
    if (hasAccess('restaurants', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              ...
              {hasAccess('restaurants', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
              ...
              {hasAccess('restaurants', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="restaurants" />
        </Flex>

        {hasAccess('restaurants', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
