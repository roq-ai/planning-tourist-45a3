...
import { getBusinessHours } from 'apiSdk/business-hours';
import { BusinessHoursInterface } from 'interfaces/business-hours';
import { getLocalCuisines } from 'apiSdk/local-cuisines';
...
import { LocalCuisineListPage as ShowCaseList } from 'pages/local-cuisines';

function DashboardPage() {
  const { session } = useSession();

  const businessHoursStat = useSWR<PaginatedInterface<BusinessHoursInterface>>(
    () => `/business-hours`,
    () => getBusinessHours({ limit: 1, offset: 0 }),
  );

  const localCuisineStat = useSWR<PaginatedInterface<LocalCuisineInterface>>(
    () => `/local-cuisines`,
    () => getLocalCuisines({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: businessHoursStat?.data?.totalCount,
      isLoading: businessHoursStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Local Cuisines',
      result: localCuisineStat?.data?.totalCount,
      isLoading: localCuisineStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Local Cuisines
                </Text>
                <NextLink href={`local-cuisines`} passHref legacyBehavior>
                  <Button
